...
function main() {
  ...
  checkNode();
  if (checkVercel()) { log('Vercel CLI already installed'); console.log(JSON.stringify({ status: 'already_installed' })); process.exit(0); }
  ...
  else { log('Error: vercel command not found after installation'); process.exit(1); }
}
main();
